...
and the value of each node is between 0 and 10,000,000,000
...
# we are effectively looking for the tree's depth
# apply BFS
def visible_nodes(root):
  ...
  while queue:
    ...
    # subtract one after we've popped it from queue (ie we've seen this node)
    nodes_at_level -= 1

    # if nodes_at_level == 0, we look at the next sets of nodes in the queue
    # these are our nodes added in the next level down
    if nodes_at_level == 0:
      depth += 1
